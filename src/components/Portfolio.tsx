...
const Portfolio: React.FC = () => {
  ...
  return (
  <section id="portfolio" className="py-20 bg-gradient-luxury">
      <div className="container mx-auto px-6">
        <h2 className="font-display text-4xl md:text-5xl font-bold mb-10 text-center text-elegant-gray">Mi Trabajo</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {dresses.slice(0, visible).map((dress, idx) => (
            <div key={idx} className="bg-card/80 rounded-lg shadow-lg overflow-hidden flex flex-col items-center p-6">
              ...
              <h3 className="font-display text-2xl font-semibold text-elegant-gray mb-2">{dress.title}</h3>
              <p className="text-base md:text-lg font-medium text-soft-gray/90 leading-relaxed" style={{fontWeight: 500}}>{dress.description}</p>
            </div>
          ))}
        </div>
        {hasMore && (
